...
# This reads the final relaxed structure from 3csic_relaxed.out, merges it with the parameters of 3csic_conventional.in,
# and writes a new 3csic_scf.in file with calculation = 'scf' and updated nat = 8.
echo "4. Extracting relaxed structure from PWSCF output and creating scf input..."
cellify -i 3csic_relaxed.out --template 3csic_conventional.in -o 3csic_scf.in --calc scf
...
echo "  - 3csic_scf.in             (8 atoms, extracted from relax.out using template, calculation='scf')"
