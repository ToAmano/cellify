...
- **Supercell Generation**: Demonstrates how to select a specific structure index non-interactively using `--select` (or interactively if `--select` is omitted in a terminal session) and pipe it directly into `cellify`'s modeling pipeline (e.g. conventionalizing the unit cell and creating a 2x2x2 supercell).
